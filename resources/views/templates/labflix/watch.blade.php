...
<div class="py-80 bg-cinema-dark">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="main-video--wrapper cinema-player-card overflow-hidden rounded-4 shadow-2-strong">
                    <div class="main-video">
                        <video class="video-player w-100" playsinline controls
                        ...
                        @if ($item->version == Status::RENT_VERSION && !$watchEligible)
                            <div class="main-video-lock">
                                <div class="main-video-lock-content glass-lock">
                                    <span class="icon animate-pulse"><i class="las la-lock"></i></span>
                                    <p class="title text-uppercase fw-bold tracking-wider">@lang('Premium Content')</p>
                                    <p class="price rounded-pill">
                                        <span class="price-amount">{{ showAmount($item->rent_price) }}</span>
                                        <span class="small-text ms-2">@lang('for') {{ $item->rental_period }} @lang('Days')</span>
                                    </p>
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="ad-video position-relative d-none">
                        ...
                        <div class="d-flex justify-content-between align-items-center flex-wrap skip-video p-3">
                            <span class="advertise-text d-none">@lang('Advertisement') - <span class="remains-ads-time">00:52</span></span>
                            <button class="skipButton d-none btn btn-sm btn-outline-light" id="skip-button" data-skip-time="0">@lang('Skip Ad') <i class="las la-forward"></i></button>
                        </div>
                    </div>

                    <div class="movie-content p-4 bg-glass-dark">
                        <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
                            <div class="movie-content-left">
                                <h2 class="movie-main-title mb-1">{{ __($item->title) }}</h2>
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb mb-0">
                                        <li class="breadcrumb-item text-danger fw-bold">@lang('Category')</li>
                                        <li class="breadcrumb-item active text-white opacity-75">{{ @$item->category->name }}</li>
                                        @if ($item->sub_category)
                                            <li class="breadcrumb-item active text-white opacity-75">{{ @$item->sub_category->name }}</li>
                                        @endif
                                    </ol>
                                </nav>
                            </div>
                            
                            <div class="movie-content-right text-sm-end">
                                <div class="d-flex align-items-center justify-content-sm-end gap-2 flex-wrap mb-3">
                                    @auth
                                        @if ($watchEligible && gs('watch_party'))
                                            <button type="button" class="btn btn-sm btn-party watchPartyBtn rounded-pill">
                                                <i class="las la-desktop me-1"></i><span>@lang('Watch Party')</span>
                                            </button>
                                        @endif
                                    @endauth
                                    ...
                                    <div class="widget-pill">
                                        ...
                                        <span>{{ getShortAmount($item->view) }}</span>
                                    </div>

                                    @php $wishlist = $item->wishlists->where('user_id', auth()->id())->count(); @endphp
                                    <button class="btn-wishlist addWishlist {{ $wishlist ? 'd-none' : '' }}" data-id="{{ $item->id }}" data-type="item">
                                        <i class="las la-plus-circle"></i>
                                    </button>
                                    <button class="btn-wishlist text-danger removeWishlist {{ $wishlist ? '' : 'd-none' }}" data-id="{{ $item->id }}" data-type="item">
                                        <i class="las la-minus-circle"></i>
                                    </button>
                                </div>

                                <ul class="post-share-modern list-unstyled d-flex gap-3 justify-content-sm-end m-0">
                                    <li><a title="Facebook" href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}" target="_blank"><i class="lab la-facebook-f"></i></a></li>
                                    <li><a title="Twitter" href="https://twitter.com/intent/tweet?text={{ __(@$item->title) }}%0A{{ url()->current() }}" target="_blank"><i class="lab la-twitter"></i></a></li>
                                    <li><a title="Pinterest" href="http://pinterest.com/pin/create/button/?url={{ urlencode(url()->current()) }}&description={{ __(@$item->title) }}&media={{ getImage(getFilePath('item_landscape') . '/' . @$item->image->landscape) }}" target="_blank"><i class="lab la-pinterest"></i></a></li>
                                </ul>
                            </div>
                        </div>
                        <p class="mt-4 preview-text-modern text-muted">{{ __($item->preview_text) }}</p>
                    </div>
                </div>

                <ul class="nav cinema-tabs mt-5 gap-4" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link active" data-bs-toggle="tab" href="#product-desc-content" role="tab">@lang('Synopsis')</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" data-bs-toggle="tab" href="#product-team-content" role="tab">@lang('Cast & Crew')</a>
                    </li>
                </ul>

                <div class="tab-content cinema-tab-content p-4 rounded-bottom-4">
                    <div class="tab-pane fade show active" id="product-desc-content" role="tabpanel">
                        <div class="lh-lg opacity-75">{{ __($item->description) }}</div>
                    </div>
                    <div class="tab-pane fade" id="product-team-content" role="tabpanel">
                        <div class="row gy-3">
                            <div class="col-md-6 col-lg-4">
                                <div class="team-item"><span class="label">@lang('Director')</span><span class="val">{{ __($item->team->director) }}</span></div>
                            </div>
                            <div class="col-md-6 col-lg-4">
                                <div class="team-item"><span class="label">@lang('Producer')</span><span class="val">{{ __($item->team->producer) }}</span></div>
                            </div>
                            <div class="col-md-12">
                                <div class="team-item"><span class="label">@lang('Starring')</span><span class="val">{{ __($item->team->casts) }}</span></div>
                            </div>
                            <div class="col-md-6 col-lg-4">
                                <div class="team-item"><span class="label">@lang('Genre')</span><span class="val">{{ __( @$item->team->genres ) }}</span></div>
                            </div>
                            <div class="col-md-6 col-lg-4">
                                <div class="team-item"><span class="label">@lang('Language')</span><span class="val">{{ __( @$item->team->language ) }}</span></div>
                            </div>
                        </div>
                    </div>
                </div>
...
<section class="movie-section pb-80">
    <div class="container">
        <div class="section-header d-flex justify-content-between align-items-center mb-4">
            <h2 class="section-title-modern">@lang('More Like This')</h2>
            <div class="header-line flex-grow-1 ms-4 d-none d-md-block"></div>
        </div>
        <div class="row g-3 justify-content-center">
            @foreach ($relatedItems as $related)
                <div class="col-xl-2 col-lg-3 col-md-4 col-6">
                    <div class="modern-movie-card" data-badge="{{ $related->versionName }}">
                        <div class="thumb">
                            <img src="{{ getImage(getFilePath('item_portrait') . '/' . $related->image->portrait) }}" alt="poster">
                            <a class="play-overlay" href="{{ route('watch', $related->slug) }}">
